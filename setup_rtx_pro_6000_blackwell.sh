...
MIN_CUDA_MAJOR="${MIN_CUDA_MAJOR:-12}"
MIN_CUDA_MINOR="${MIN_CUDA_MINOR:-8}"
CLEAN_BUILD="${CLEAN_BUILD:-1}"

# Single-user inference profile.
PARALLEL="${PARALLEL:-1}"
SPEC_TYPE="${SPEC_TYPE:-draft-mtp}"
SPEC_DRAFT_N_MAX="${SPEC_DRAFT_N_MAX:-4}"
SPEC_DRAFT_P_MIN="${SPEC_DRAFT_P_MIN:-0.5}"
CACHE_REUSE="${CACHE_REUSE:-256}"
USE_NO_HOST="${USE_NO_HOST:-1}"
...
validate_config() {
  ...
  [[ "$BATCH" =~ ^[1-9][0-9]*$ ]] || die "BATCH deve essere un intero > 0."
  [[ "$UBATCH" =~ ^[1-9][0-9]*$ ]] || die "UBATCH deve essere un intero > 0."
  (( UBATCH <= BATCH )) || die "UBATCH=$UBATCH non può superare BATCH=$BATCH."
...
cuda_preflight() {
  ...
  have nvcc || die "nvcc non trovato. Per RTX PRO 6000 Blackwell usa una RunPod image CUDA 12.8+ *devel*."

  local cuda_ver cuda_major cuda_minor gpu_name
  cuda_ver="$(nvcc --version | sed -n 's/.*release \([0-9][0-9]*\.[0-9][0-9]*\).*/\1/p' | tail -n1)"
  [[ -n "$cuda_ver" ]] || die "Impossibile determinare la versione CUDA da nvcc --version."

  cuda_major="${cuda_ver%%.*}"
  cuda_minor="${cuda_ver#*.}"
  cuda_minor="${cuda_minor%%.*}"

  if (( cuda_major < MIN_CUDA_MAJOR || (cuda_major == MIN_CUDA_MAJOR && cuda_minor < MIN_CUDA_MINOR) )); then
    die "CUDA $cuda_ver troppo vecchio per SM120. Serve CUDA ${MIN_CUDA_MAJOR}.${MIN_CUDA_MINOR}+."
  fi

  gpu_name="$(nvidia-smi --query-gpu=name --format=csv,noheader | head -n1 || true)"
  log "GPU: ${gpu_name:-sconosciuta}"
  log "CUDA toolkit: $cuda_ver | target CMake: SM$CUDA_ARCH"

  nvidia-smi --query-gpu=name,driver_version,memory.total --format=csv,noheader || true
  nvcc --version | tail -n 4 || true
  ok "GPU NVIDIA + CUDA ${cuda_ver} compatibile"
}
...
build_llama() {
  ...
  if [[ "$CLEAN_BUILD" == "1" && -d "$LLAMA_DIR/build" ]]; then
    log "Clean rebuild: rimuovo $LLAMA_DIR/build"
    rm -rf "$LLAMA_DIR/build"
  fi

  log "Configuro llama.cpp: CUDA Release + Ninja + Blackwell SM${CUDA_ARCH}"
  ...
  ok "Build CUDA SM${CUDA_ARCH} completata"
}
...
sanity_check() {
  ...
  for required_flag in '--spec-type' '--spec-draft-n-max' '--spec-draft-p-min' '--no-host' '--cache-reuse'; do
    grep -q -- "$required_flag" <<<"$server_help" ||       die "Questa build di llama-server non supporta $required_flag. Aggiorna/rebuilda llama.cpp."
  done
  ok "Flag runtime MTP/no-host supportati"
